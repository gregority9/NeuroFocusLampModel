...
Oprócz EEG projekt uwzględnia też dane z akcelerometru. Na początku akcelerometr powinien być używany przede wszystkim jako pomoc w wykrywaniu artefaktów ruchowych, kontroli jakości oraz oznaczaniu podejrzanych okien. Nie trzeba od razu używać akcelerometru jako głównego źródła cech klasyfikacyjnych, ale pipeline powinien być przygotowany tak, żeby dało się to łatwo dodać później.
...
|---|---|---|
| **Grzechu** | Preprocessing + QC | Przygotowanie oczyszczonych sygnałów EEG, synchronizacja akcelerometru oraz raporty jakości |
...
Grzechu odpowiada za przygotowanie danych EEG i danych z akcelerometru do dalszego przetwarzania.
...
- wykorzystanie akcelerometru do detekcji ruchu,
...
Przykładowy format wejściowy EEG:

```text
timestamp, AF3, AF4, F3, F4, FC5, FC6, O1, O2
```

Jeżeli dane z akcelerometru są w tym samym pliku, przykładowy format może wyglądać tak:

```text
timestamp, AF3, AF4, F3, F4, FC5, FC6, O1, O2, acc_x, acc_y, acc_z
```

Jeżeli dane z akcelerometru są w osobnym pliku, powinny być synchronizowane po czasie lub indeksie próbki.
...
- dostępność danych akcelerometru,
- zgodność timestampów EEG i akcelerometru,
- nietypowo duży ruch w trakcie nagrania.
...
### 1.5. Artefakty EEG + akcelerometr

Na start należy przygotować podstawowy artifact scoring łączący informacje z EEG i akcelerometru.

Przykładowe kryteria EEG:
...
Przykładowe kryteria z akcelerometru:

- duża wartość normy przyspieszenia,
- nagły skok `acc_x`, `acc_y` lub `acc_z`,
- wysoka wariancja akcelerometru w oknie,
- duża energia ruchu w oknie,
- korelacja ruchu z anomaliami w EEG,
- oznaczanie okien z ruchem głowy jako potencjalnie artefaktowych.

Przykładowe cechy pomocnicze:

```text
acc_norm = sqrt(acc_x^2 + acc_y^2 + acc_z^2)
acc_delta = abs(diff(acc_norm))
acc_rms
acc_variance
acc_peak_to_peak
motion_score
...
Akcelerometr powinien przede wszystkim pomagać w oznaczaniu artefaktów, np.:

```text
is_motion_artifact = motion_score > threshold
```
...
preprocessing:
  ...
  accelerometer:
    ...
    motion_z_threshold: 4.0
    use_for_artifact_score: true
```
...
data/interim/sub-001/rest_acc_features.csv
data/interim/sub-001/focus_memory_acc_features.csv
data/interim/sub-001/focus_words_acc_features.csv
...
Szymon odpowiada za zamianę oczyszczonego sygnału EEG oraz informacji z akcelerometru na dane gotowe do trenowania modeli.
...
- cechy ruchowe z akcelerometru,
- warianty modeli EEG-only oraz EEG + accelerometer.
...
### 2.7. Cechy z akcelerometru

Na start akcelerometr powinien być traktowany głównie jako źródło informacji o jakości okna, a nie jako główna cecha do klasyfikacji stanu poznawczego.

Dla każdego okna warto policzyć:

```text
acc_norm_mean
acc_norm_std
acc_norm_max
acc_norm_peak_to_peak
acc_delta_mean
acc_delta_max
motion_score
```

Te cechy powinny trafić do metadanych okien i raportów QC. Opcjonalnie można przygotować osobny wariant tabeli cech:

```text
features_eeg_only.csv
features_eeg_plus_acc.csv
```

Ważne: jeśli akcelerometr będzie używany jako cecha modelu, trzeba jasno opisać eksperyment, bo model może zacząć rozpoznawać ruch lub artefakty zamiast stanu poznawczego.
...
- motion score relaks vs zadanie,
- zależność między motion score a artifact score,
- sprawdzenie, czy `focus_words` nie ma więcej ruchu przez mowę, napięcie twarzy albo mikroruchy głowy.
...
features:
  ...
  use_asymmetry: true
  use_covariance: true
  use_accelerometer_features: false
```
...
data/processed/features_eeg_only.csv
data/processed/features_eeg_plus_acc.csv
data/processed/covariance_matrices.npy

reports/figures/psd_rest_vs_focus.png
reports/figures/bandpower_summary.png
reports/figures/motion_score_summary.png
```
...
- warianty eksperymentów EEG-only oraz EEG + accelerometer.
...
### 3.7. Kontrola artefaktów z użyciem akcelerometru
...
- korelacja `motion_score` z predykcją,
- porównanie wyników przed i po odrzuceniu artefaktów,
- porównanie EEG-only vs EEG + accelerometer,
- sprawdzenie, czy model nie wykorzystuje samego ruchu jako skrótu decyzyjnego.

Celem jest sprawdzenie, czy model nie klasyfikuje głównie artefaktów, ruchu głowy albo napięcia mięśniowego.
...
artifacts:
  ...
  reject_motion_artifacts: true

features:
  input_variant: eeg_only
  # possible values:
  # eeg_only
  # eeg_plus_acc
```
...
```text
1. Grzechu:
   loader, walidacja, filtrowanie, QC, synchronizacja akcelerometru

2. Szymon:
   okienkowanie, etykiety, bandpower, motion score, tabela cech
...
```text
1. Grzechu:
   artifact scoring z EEG i akcelerometru oraz lepsze raporty jakości

2. Szymon:
   ratio features, asymetrie, covariance features, cechy ruchowe, PSD sanity check

3. Wiciu:
   Linear SVM, Riemannian pipeline, EEG-only vs EEG + accelerometer, raporty eksperymentów
```
...
Ma umożliwić łatwe dodanie nowych filtrów, metod referencji, kanałów, artefaktów i wykorzystania akcelerometru.

Szymon:
Modułowe okienkowanie i cechy.
Ma umożliwić łatwe dodanie nowych długości okien, pasm, cech, agregatów, macierzy kowariancji i cech ruchowych.

Wiciu:
Modułowe modele i eksperymenty.
Ma umożliwić łatwe dodanie nowych modeli, walidacji, metryk, raportów, eksperymentów kontrolnych oraz porównań EEG-only vs EEG + accelerometer.
```
...
MVP teraz
-> więcej cech później
-> lepsze wykorzystanie akcelerometru później
