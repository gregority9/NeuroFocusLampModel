...
def load_riemannian_data(config):
    covariance_file = config["data"]["covariance_file"]
...
def run_riemannian_loso_training(X, metadata_df, config):
    ...
    for fold_id, split in enumerate(logo.split(X, y, groups)):
        ...
        model = build_riemannian_logreg_pipeline(config)
...
def train_final_riemannian_model(X, metadata_df, config):
    """Train one final Riemannian model on all covariance matrices."""
    target_column = config["data"]["target_column"]
...
def main():
    ...
    X, metadata_df = load_riemannian_data(config)
    validate_riemannian_data(X, metadata_df, config)

    results_df, predictions_df, feature_importance_df = run_riemannian_loso_training(
        X,
        metadata_df,
        config
    )
    ...
    final_model = train_final_riemannian_model(X, metadata_df, config)
    save_model_artifacts(final_model, [], config, output_dir, input_format="covariance_matrices")
